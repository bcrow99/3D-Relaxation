...
public class Test2
{
	...
	public static void main(String[] args)
	{
		if (args.length != 3)
		{
			System.out.println("Usage: java Test2 <intensity> <difference> <resolution>");
			System.exit(0);
		}
		...
		Test2 test       = new Test2(intensity, difference, resolution, threshold);
	}
	
	public Test2(double intensity, int difference, int resolution, double threshold)
	{
        ...
        for(int i = 0; i < resolution; i++)
		{
			for(int j = 0; j < resolution; j++)
			{
				...
				if(blue[k] == 255)
				{
					...
					double limit = (double)value_list.get(m);
					double value = plane[i][j];
					while(value > limit)
					{
						m += increment;
						if(m > value_list.size() - 1)
							break;
						limit = (double)value_list.get(m);
						n++;
					}
					blue[k]  = n;
					green[k] = blue[k];
